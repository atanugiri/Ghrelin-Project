...
% buildLongTable2way
% Returns a long-format table for 2-way ANOVA collapsed across tasks:
%   Y, Group={Saline,Ghrelin}, Dreadds={WT,Inhibitory,Excitatory}
%
% Inputs:
%   normType    : 1=raw (default), 2=z-score vs col1 (per file), 3=min-max (per file)
...
%   varargin    : one or more CSV/XLSX files; each file has 6 columns in order:
%                 [Saline-WT, Ghrelin-WT, Saline-Inhib, Ghrelin-Inhib, Saline-Excit, Ghrelin-Excit]
%
% Notes:
%   • Multiple files are pooled together; there is NO Task factor here.
%   • Normalization (when requested) is performed per file, then data are pooled.
%
% Output:
%   longTbl : table with variables Y, Group, Dreadds
...
for fi = 1:numel(varargin)
    ...
    % Coerce first 6 columns to numeric vector columns
    X = zeros(height(Ti), 6);
    for c = 1:6
